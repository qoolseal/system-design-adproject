...
- Active–Standin между Москвой (мастер) и Новосибирском (read-only).
#### Геораспределение
- Основной ЦОД — **Москва** (активный).
- Резервный — **Новосибирск** (read-only, репликация данных, резервное хранение).
- GeoDNS работает **как умный балансировщик** 
	- В штатном режиме всегда отдаёт IP МСК.
	- В аварии (по health-check) или при плановой операции (по API) - отдаёт IP НСК.
- При плановом переводе можно регулировать веса для плавного включения НСК.
#### PostgreSQL
- Модель `global master + read replicas`.
	- репликация через kafka, 
	- Transactional Outbox: события саги пишутся в outbox, фоновый консьюмер читает их и отправляет события в топики
...
#### Kafka
- Используется **геораспределённый Kafka-кластер** с 2+1 брокерами (два в Москве, один в Новосибирске), RF = 3
- Каждое событие, критичное для бизнес-процессов (например, создание заказа, оплата), публикуется в Kafka и попадает в оба региона за счёт кворума и репликации.
- Это делает **стриминг PostgreSQL между ЦОДами ненужным** - все данные, которые нужно синхронизировать, передаются через события Kafka, а уже из них обновляются локальные БД и ElasticSearch в каждом ЦОД.
...
- геораспределённый кластер
	- 2 брокера в мск
	- 1 брокер в нск - только реплицирует события из мск-брокеров
- при обвале ЦОД-1: 
	-  **МСК недоступен.** Контроллер-кворум потерян → Kafka-кластер «замирает».
	- **Сервисы в НСК** продолжают:
	    - **Чтение** из локальных **ES/Redis** (поиск/просмотр работают).
	    - **Запись бизнес-действий** складывают в **Transactional Outbox** в своих БД (или локальные очереди).
	- После восстановления МСК:
	    - Outbox-процессоры «досылают» отложенные события в Kafka.
	    - Реплики догоняются; система возвращается в обычный режим.
	    - Благодаря репликам в НСК **RPO≈0** для подтверждённых записей до аварии.
**плюсы**
- При **падении одного брокера** или проблемах связи — кластер продолжает работать; данные уже реплицируются и есть копии в НСК.
- **Потребители в НСК** могут читать из «своей» реплики (client.rack), снижая межрегиональный трафик/латентность.
- **RPO≈0** при восстановлении МСК: все подтверждённые записи уже лежат как минимум на двух репликах в разных ЦОДах (при `acks=all`, `min.insync.replicas=2`).