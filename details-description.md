...
### Масштабирование
Так как у нас микросервисная система с асинхронной коммуникацией через Kafka, то она может масштабироваться горизонтально. Используем Kubernetes
- Микросервисы stateless, используем балансировщик нагрузки для распределения запросов по инстансам микросервисов
	- Kubernetes следит за нагрузкой (CPU, память) и автоматически запускает больше копий (autoscaling) нужного сервиса.
	- В Kubernetes разворачиваются `ad-service`, `user-service`, `order-service`, `payment-service`, `search-service` и т.д.; API Gateaway, search-indexer (kafka consumer, обновляющий elastic), сервисы модерации, prometheus + grafana
	- Вне k8s: Postgres, Kafka, Redis, Elastic
- Elasticsearch можно масштабировать горизонтально, добавляя новые ноды. Использовать индексы по регионам. Автоматически шардирование использует
...
- в Postgres используем read-реплики, шардирование по регионам, чтобы не подгружать дальние объявление
	- Чтение в регионах осуществляется с read-реплик, запись — централизованная. В будущем возможен переход на физическое шардирование.
...
- **PostgreSQL** работает в модели `global master + regional read replicas`, где записи+чтение выполняются в Москве, а Новосибирск обслуживает только чтение.
	- для записи из ЦОД нск pgPool выдаёт подключение к мастеру (мск)
...
- Включается **резервная копия PostgreSQL-мастера** (если доступна) или пишем во временное хранилище / буфер (outbox).
