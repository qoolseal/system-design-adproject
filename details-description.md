...
- [версия №4](https://viewer.diagrams.net/?tags=%7B%7D&lightbox=1&highlight=0000ff&edit=_blank&layers=1&nav=1&title=%D1%81%D0%B8%D1%81%D1%82%D0%B5%D0%BC%D0%B0%20%D0%BE%D0%B1%D1%8A%D1%8F%D0%B2%D0%BB%D0%B5%D0%BD%D0%B8%D0%B9%20%D1%81%20%D0%B4%D0%BE%D1%81%D1%82%D0%B0%D0%B2%D0%BA%D0%BE%D0%B9%20%D0%B8%20%D0%BE%D0%BF%D0%BB%D0%B0%D1%82%D0%BE%D0%B9&dark=auto#Uhttps%3A%2F%2Fdrive.google.com%2Fuc%3Fid%3D1ZmRJ5LRRFAmD-se9SyyQ4232OCjZHaLY%26export%3Ddownload) 10.08
...
- Kubernetes: liveness/readiness-пробы, развёртывание сервисов в нескольких зонах доступности внутри московского ЦОДа.
- Kafka, Redis, Elasticsearch — с репликацией данных и распределением по зонам.
- DNS: настраивается с пониженным TTL, чтобы в случае аварии можно было вручную переключить трафик на резервную площадку в Новосибирске после её запуска.
#### Геораспределение
- Основной и рабочий ЦОД — Москва (активный, мультизонный).
- Резервный — Новосибирск (холодный резерв: хранятся бэкапы баз данных, снимки Elasticsearch и Redis, артефакты для развёртывания сервисов).
- В обычной работе используется только московский ЦОД. В случае полной потери Москвы по инструкции разворачивается система в Новосибирске и вручную переключается трафик.
#### PostgreSQL
- Модель: master сервер и реплики в разных зонах московского ЦОДа.
- Для повышения отказоустойчивости одна из реплик работает в синхронном режиме, чтобы при отказе ведущего данные не терялись.
- Используются регулярные резервные копии и архивирование журналов изменений (WAL), которые дублируются в Новосибирск.
- Patroni управляет автоматическим выбором ведущего внутри московского кластера.
- etcd для согласования лидера.
#### Redis
- Кластер Redis работает в Москве с репликацией между зонами.
- Fallback в БД при холодном кэше.
#### Kafka
- Единый кластер Kafka развёрнут в Москве, брокеры распределены по зонам доступности.
- Настроена репликация сообщений между брокерами, подтверждения записи от большинства узлов и идемпотентные продюсеры.
- Для восстановления в случае аварии резервные копии данных и метаданных кластера выгружаются в хранилище, доступное также в Новосибирске.
...
- для сервисов поиска и отображения объявлений важнее доступность: данные берутся из реплик и кэшей, допускается некоторая задержка обновления.
...
- **Redis** развёрнут в московском ЦОДе, узлы распределены по зонам доступности. Это обеспечивает низкие задержки и устойчивость при отказе одной зоны.
...
### 2.4 Обеспечение высокой доступности
#### Общие меры
- Все микросервисы развёрнуты в нескольких зонах московского ЦОДа. Kubernetes автоматически перезапускает контейнеры и распределяет нагрузку.
...
3. **Отказ PostgreSQL-мастера:**
    - Переключение на реплику выполняется автоматически (Patroni).
    - Подключения приложений перенастраиваются через pgbouncer.

4. **Недоступность Redis:**
    - Временно отключается кэширование. Система работает напрямую с базой данных или Elasticsearch.

5. **Потеря одной зоны в московском ЦОДе:**
    - Сервисы продолжают работу в других зонах. Потеря доступности для пользователей минимальна или незаметна.

6. **Полная потеря московского ЦОДа:**
    - Система становится временно недоступной.
    - По инструкции разворачивается резерв в Новосибирске: поднимается база из бэкапов, восстанавливаются поисковые индексы, запускаются сервисы.
    - После проверки трафик вручную переключается на резервную площадку.

#### Резервная площадка (Новосибирск)
В Новосибирске развёрнут холодный резерв. Там нет постоянно работающих сервисов, но подготовлена инфраструктура для восстановления системы при полной потере московского ЦОДа.
##### Хранятся:
- Резервные копии баз данных PostgreSQL и архивы журналов изменений (WAL).
- Снимки индексов Elasticsearch и дампы данных Redis.
- Резервные копии и метаданные кластера Kafka.
- Артефакты для развёртывания сервисов (образы контейнеров, Helm-чарты, конфигурации).
- Статические файлы и медиа — копируются в объектное хранилище с доступом из Новосибирска.
##### Процедура восстановления:
1. Поднятие сетевой и базовой инфраструктуры (k8s-кластер, хранилища).
2. Восстановление баз данных из бэкапов и журналов изменений.
3. Развёртывание кластера Kafka из сохранённых метаданных.
4. Восстановление поисковых индексов и кэшей из снимков.
5. Развёртывание микросервисов и проверка их работоспособности.
6. Переключение пользовательского трафика в Новосибирск вручную после успешной проверки.

Таким образом, резервная площадка обеспечивает возможность возобновить работу системы даже в случае полной потери московского ЦОДа, но требует ручного запуска и времени на восстановление.
...
## Этап 3. Связь микросервисов

### Kafka
- Кластер Kafka развёрнут в московском ЦОДе, брокеры распределены по разным зонам доступности.
- Настроена репликация сообщений между брокерами (Replication Factor = 3), подтверждение записи от большинства узлов (`acks=all`, `min.insync.replicas=2`) и идемпотентные продюсеры.
- Это позволяет при отказе отдельного брокера или зоны продолжать работу без потери данных.
- Резервные копии и метаданные кластера периодически сохраняются в объектное хранилище, дублируемое также в Новосибирске. При полной потере московского ЦОДа кластер может быть развёрнут заново из этих резервных копий.
**Плюсы**
- При падении одного брокера или даже целой зоны кластер продолжает работать.
- Репликация сообщений между брокерами обеспечивает надёжное хранение и минимальный риск потери данных.
- В случае аварии в Москве возможно восстановление работы Kafka в Новосибирске из сохранённых резервных копий.