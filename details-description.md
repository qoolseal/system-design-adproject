...
- GeoDNS работает **как умный балансировщик** — перенаправляет трафик в другой ЦОД только при аварии.
- Пользователи из всех регионов в штатной ситуации обслуживаются в Москве.
- Переключение выполняется **плавно** (балансировка весов, частичный DNS-rotation, Anycast/BGP), без имитации резкого отказа.
#### PostgreSQL
