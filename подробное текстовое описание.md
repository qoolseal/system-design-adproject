...
- Будем кэшировать популярные объявления в Redis. Так как они часто запрашиваются - прогреваются при старте приложения, а так же используем read-through кэш. Динамические данные кэшируются по событиям AdViewed, AdUpdated
